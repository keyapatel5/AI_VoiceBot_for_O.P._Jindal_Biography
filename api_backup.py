...
class Fallback:
    def __init__(self):
        ...
        self.hi = {
            "general": [
                "कृपया मुझसे ओ.पी. जिंदल के जीवन, परिवार, व्यापार या उपलब्धियों के बारे में पूछें!",
                "यह मेरी विशेषज्ञता के बाहर है। ओ.पी. जिंदल के बारे में पूछें!",
                "मुझे इस बारे में जानकारी नहीं है! ओ.पी. जिंदल के बारे में पूछें!",
                "ओ.पी. जिंदल के बारे में कुछ भी पूछें, मैं जवाब देने की कोशिश करूंगा!",
            ],
            "family": [
                "कृपया ओ.पी. जिंदल के पत्नी, बच्चों या माता-पिता के बारे में पूछें!",
            ],
            "business": [
...
# ============================================================
# CACHE  (SPEED: use plain string key, skip MD5 on short queries)
# ============================================================
...
class Cache:
    ...
    @staticmethod
    def _key(t: str) -> str:
        # SPEED: skip MD5 for short queries — direct lowercase key is fast enough
        # and avoids hashing overhead on every lookup
        s = t.lower().strip()
        if len(s) <= 120:
            return s
...
class Engine:
    def __init__(self, name):
        ...
        # SPEED: pre-compile norm substitution patterns once
        self._norm_pats = [
            (re.compile(r"om\s+prakash\s+jindal"), "op jindal"),
            (re.compile(r"o\.?\s*p\.?\s+jindal"),  "op jindal"),
            (re.compile(r"\bo\s+p\b"),              "op"),
            (re.compile(r"o\.p\."),                 "op"),
            (re.compile(r"o\s*p\s*jindal"),         "op jindal"),
        ]
    ...
    def search(self, qn_str, qt_list, boost=False, qe_precomputed=None):
        """
        SPEED: accept a pre-computed query embedding so dual-language
        searches don't encode the same string twice.
        """
        with self._lock:
        ...
        # SPEED: reuse pre-computed embedding if provided
        qe  = qe_precomputed if qe_precomputed is not None \
              else embed_model.encode(qn_str, convert_to_tensor=True)
...
def search(query, stt_lang: str = None):
    ...
    _emb_cache: dict = {}
    def _emb(s: str):
        if s not in _emb_cache:
            _emb_cache[s] = embed_model.encode(s, convert_to_tensor=True)
        return _emb_cache[s]

    def _best_en():
        e1 = _emb(en_n)
        a1, s1, _ = eng_en.search(en_n, en_t, boost=True, qe_precomputed=e1)
        if a1 and s1 >= EARLY_EXIT:          # SPEED: skip second search if already great
            return a1, s1
        e2 = _emb(tr_n)
        a2, s2, _ = eng_en.search(tr_n, tr_t, boost=True, qe_precomputed=e2)
        if a1 and a2:
    ...
    if lang == "hi":
        e_hi = _emb(hi_n)
        a_hi, s_hi, _ = eng_hi.search(hi_n, hi_t, boost=True, qe_precomputed=e_hi)
        ...
        e_en = _emb(en_n)
        a_en, s_en, _ = eng_en.search(en_n, en_t, boost=True, qe_precomputed=e_en)
        log.info(f"EN_RESULT: {s_en:.3f} | {str(a_en)[:50] if a_en else '-'}")
        if a_en and s_en >= EARLY_EXIT:
            return a_en, s_en, "english_db", lang
        e_hi = _emb(hi_n)
...
app = FastAPI(title="JindalBot", version="9.1")
app.add_middleware(
...
# ============================================================
# STT  (SPEED: beam_size=1 for first pass; retry only on low confidence)
# ============================================================

def _transcribe_sarvam(wav_path):
    """Try Sarvam Saaras v3 codemix API. Returns (text, lang) or (None, None)."""
    try:
        t0 = time.time()
        with open(wav_path, "rb") as f:
            fname = os.path.basename(wav_path) or "audio.wav"
            resp = requests.post(
                SARVAM_API_URL,
        ...
        if resp.status_code != 200:
            log.warning(f"Sarvam API {resp.status_code}: {resp.text[:200]}")
            return None, None

        data = resp.json()
        text = (data.get("transcript") or "").strip()
        if not text or len(text) < 2:
            log.warning(f"Sarvam empty transcript ({ms:.0f}ms)")
            return None, None
...
# ============================================================
# TTS  (SPEED: skip disk write — stream directly to bytes in memory)
# ============================================================
...
async def tts(text, lang_hint: str = None):
    try:
        ...
        if chunks:
            ...
            log.info(f"TTS: voice={voice} bytes={len(audio_bytes)}")
...
async def chat(req: In):
    ...
    t0 = time.time()
    # SPEED: run CPU-bound answer() in thread pool so event loop stays free
    loop = asyncio.get_event_loop()
    r    = await loop.run_in_executor(_CPU_POOL, lambda: answer(req.text, req.session_id))
...
async def voice_chat(file: UploadFile = File(...)):
    ...
    try:
        ...
        # SPEED: run ffmpeg without blocking the async event loop
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(
            None,   # default thread pool — ffmpeg is I/O bound
        ...
        # SPEED: transcribe + answer both CPU-bound; run in executor
        ut, stt_lang = await loop.run_in_executor(_CPU_POOL, transcribe, wav)
        ...
        r  = await loop.run_in_executor(_CPU_POOL, lambda: answer(ut, stt_lang=stt_lang))
        a  = await tts(r["answer"], lang_hint=r["language"])
        ms = (time.time() - t0) * 1000
